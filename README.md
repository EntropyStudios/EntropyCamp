...
- 日报记录页可选择本班关联过的 Codex 对话，导出 Markdown 或 JSON；聚合同一线程的全部追加式 rollout 分片，兼容新旧消息格式，只解析班次时间窗口并缓存未变化的结果
...
主视图是 Three.js 的单太阳任务系统：中央太阳按北京时间 `08:00–24:00` 逐渐膨胀并红化，提醒卡片由 11 个《星际拓荒》天体按洗牌轮次分配，每轮内部不重复。待机任务按游戏半径 `1.5×` 低亮度运行；Codex 正在处理或普通提醒倒计时时平滑放大到 `2.25×`（相对待机 `1.5×`），并显示太阳到行星的呼吸数据链。深巨星和陌生者因视觉体量过大，在所有状态额外采用 `0.75×` 显示修正。Codex 完成或倒计时到点后，行星会变成 `1.875×` 的双层熔岩碎片云：外层保留原模型红色三角碎片，内层以 4,200 个小型实体碎片填充。右键云团会被鼠标黑洞吸收、标记已读并从白洞以新行星重生，且多个回收可连续进行。下班会触发红巨星、坍缩和超新星终局并炸毁所有行星；下次上班播放时间回溯并重新随机分配天体。视角使用无极点锁定的轨迹球控制；黑洞、白洞与太阳通过同一个屏幕空间透镜通道影响已经渲染的星场与天体。界面不绘制轨道线；WebGL 或本地模型不可用时降级为二维列表或低面占位模型。
